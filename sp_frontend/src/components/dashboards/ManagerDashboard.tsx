...
import { AlertTriangle, CheckCircle, Clock, Upload, LineChart, Settings, ImageIcon, VideoIcon, Users, Wrench } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white p-6 rounded-lg shadow-md">
        ...
        {/* Enhanced Complaints Status Summary */}
        <div className="grid grid-cols-2 md:grid-cols-6 gap-4 mb-4">
          <div className="flex items-center space-x-2">
            <Clock className="h-5 w-5 text-yellow-500" />
            <div>
              <div className="text-xs text-gray-500">Sent</div>
              <div className="font-bold text-yellow-600">{complaints.filter(c => c.status === 'sent').length}</div>
            </div>
          </div>
          <div className="flex items-center space-x-2">
            ...
            <div>
              ...
              <div className="font-bold text-orange-600">{complaints.filter(c => c.status === 'in_progress').length}</div>
            </div>
          </div>
          <div className="flex items-center space-x-2">
            ...
            <div>
              ...
              <div className="font-bold text-blue-600">{complaints.filter(c => c.status === 'unicorn_assigned').length}</div>
            </div>
          </div>
          <div className="flex items-center space-x-2">
            ...
            <div>
              ...
              <div className="font-bold text-purple-600">{complaints.filter(c => c.status === 'unicorn_received').length}</div>
            </div>
          </div>
          <div className="flex items-center space-x-2">
            ...
            <div>
              ...
              <div className="font-bold text-green-600">{complaints.filter(c => c.status === 'solved').length}</div>
            </div>
          </div>
          <div className="flex items-center space-x-2">
            ...
            <div>
              ...
              <div className="font-bold text-gray-700">{complaints.length}</div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-gray-50 p-3 rounded-lg">
          <div className="text-xs text-gray-600 mb-2">Status Guide:</div>
          <div className="grid grid-cols-2 md:grid-cols-6 gap-2 text-xs">
            <div className="flex items-center space-x-1">
              <Clock className="h-3 w-3 text-yellow-500" />
              <span>Sent - Awaiting assignment</span>
            </div>
            <div className="flex items-center space-x-1">
              <AlertTriangle className="h-3 w-3 text-orange-500" />
              <span>In Progress - Internal repair</span>
            </div>
            <div className="flex items-center space-x-1">
              <Users className="h-3 w-3 text-blue-500" />
              <span>Assigned - Sent to unicorn</span>
            </div>
            <div className="flex items-center space-x-1">
              <Wrench className="h-3 w-3 text-purple-500" />
              <span>Working - Unicorn in progress</span>
            </div>
            <div className="flex items-center space-x-1">
          ...
          <form onSubmit={handleSubmitComplaint} className="space-y-4">
            ...
            <div>
              ...
              >
                ...
                <option value="lpg">LPG - Complaints</option>
              </select>
            </div>
              ...
              <div key={complaint.id} className="border-b pb-4">
                ...
                <div className="flex justify-between items-center mt-2">
                  ...
                  <span className={`text-xs font-medium px-2 py-1 rounded-full ${
                    complaint.status === 'sent' ? 'bg-yellow-100 text-yellow-800' :
                    complaint.status === 'in_progress' ? 'bg-orange-100 text-orange-800' :
                    complaint.status === 'unicorn_assigned' ? 'bg-blue-100 text-blue-800' :
                    complaint.status === 'unicorn_received' ? 'bg-purple-100 text-purple-800' :
